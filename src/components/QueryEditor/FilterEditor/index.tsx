...
import { QueryEditorBaseRow, QueryEditorRow } from '../QueryEditorRow';
...
function filterErrors(filter: QueryFilter): string[] {
  ...
  if (
    ['term', 'not term'].includes(filter.filter.operator) &&
    filter.filter.value &&
    hasWhiteSpace(filter.filter.value)
  ) {
...
export const FilterEditor = ({ onSubmit }: FilterEditorProps) => {
  ...
  if (!filters || filters.length === 0) {
    return (
      <QueryEditorBaseRow label="Filter">
        <IconButton
          label="add"
          iconName="plus"
